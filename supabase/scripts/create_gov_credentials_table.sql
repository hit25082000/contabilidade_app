-- Script para criação da tabela gov_credentials
-- Execute este script no SQL Editor do Supabase para criar a tabela necessária

-- Verifica se a tabela gov_credentials já existe
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'gov_credentials') THEN
        ...
        RAISE NOTICE 'Tabela gov_credentials criada com sucesso!';
    ELSE
        RAISE NOTICE 'A tabela gov_credentials já existe. Nenhuma alteração foi feita.';
    END IF;
END $$; 